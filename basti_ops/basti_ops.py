...
def sort_verts_by_position(verts: list[bmesh.types.BMVert]) -> list[bmesh.types.BMVert]:
    return sorted(verts, key=lambda vert: vert.co[2], reverse=True)


def get_source_obj_and_selection(
    ...
    verts_selected = add_vertices_from_polygons(
        obj_source, verts_selected, polys_selected
    )
...
class BastiMoveAndOrientToFace(bpy.types.Operator):
    ...
    bl_label = (
        "move selected submesh or object to face under Cursor and orient to the normal"
    )
    bl_options = {"REGISTER", "UNDO"}
